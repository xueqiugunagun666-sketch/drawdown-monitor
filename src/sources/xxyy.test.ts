...
import { parseXxyyPrices, supportsChain, BATCH_SIZE } from './xxyy.ts';
...
test('地址统一小写 —— 上游大小写不定，键要能对上', () => {
  const m = parseXxyyPrices(ok([{ mint: '0xABCDEF', priceUSD: 1 }]));
...
test('价格保持字符串 —— 不让它停留在 number 上', () => {
  const m = parseXxyyPrices(ok([{ mint: '0xa', priceUSD: 0.00000000000000000001234 }]));
  assert.equal(typeof m.get('0xa')!.priceUsd, 'string');
});

test('code 非 0 时抛错，不当成空结果', () => {
  // 静默当成"这批没数据"会让所有币看起来同时失去报价
  assert.throws(() => parseXxyyPrices(JSON.stringify({ code: 401, msg: 'unauthorized' })), /401/);
});

test('data 不是数组时抛错', () => {
  assert.throws(() => parseXxyyPrices(JSON.stringify({ code: 0, data: 'nope' })), /数组/);
});

test('非 JSON 时抛错并带上原文开头，便于排查', () => {
  assert.throws(() => parseXxyyPrices('<html>502</html>'), /非 JSON/);
});
