/**
 * 钱包资格慢引擎，以及切源前报警逻辑的兼容实现。
 *
 * 生产 realPumpDeps.evaluateAlerts=false：这里只用 DexScreener 更新流动性、
 * 成交量、项目元数据与 monitored；正式暴涨/ATH 由 xxyyAlertEngine 15 秒
 * 快轮次负责。旧报警实现仍保留给回归测试与安全回滚，但生产不再调用。
 *
 * 每轮的顺序（顺序本身是有讲究的）：
