...
写这份规格期间（2026-09-02 18:02 至 09-03 00:02 北京时间之间），代币 `PONZI`（`robinhood:0xea98cb2f…d74ac`，无署名，08-30 添加）**被人删掉了**。

- 通过比对每 6 小时一次的自动备份定位到具体是哪一个、以及 6 小时的时间窗口
...
**账号 8 个**，全部有活跃会话（TTL 30 天，最晚 2026-10-02 过期）：

| 账号 | 注册时间 | 活跃会话 | 绑定钱包 | 持仓数 |
|---|---|---|---|---|
| pananiu | 08-30 22:49 | 3 | 4 | 872 |
| dadiaoxuan | 08-30 23:22 | 2 | 4 | 538 |
| retend666 | 08-30 23:23 | 2 | 0 | 0 |
| an | 08-30 23:23 | 2 | 12 | 1144 |
| an. | 08-30 23:23 | 1 | 4 | 62 |
| xx | 08-30 23:23 | 2 | 0 | 0 |
| 649257499 | 08-31 13:17 | 1 | 0 | 0 |
| 梅仁艾 | 08-31 15:52 | 1 | 0 | 0 |

- **看板代币 15 个**，全部 `visibility='public'`（09-02 时是 16 个，其中一个被删了，见上节）
- **代币署名分布**：8 个为 NULL、`小牛` 4、`retend` 2、`307大王小锐` 1
- **日历 4 个**：`小牛` 3、`赌命哥` 1
- `created_by` 只有 NULL 和非空值两种，**没有空字符串或纯空白**（已专门核查，因为回填脚本靠 `IS NULL` 判定）

关键事实：**署名与账号名是两个命名空间，对不上。** `小牛` 不对应任何账号，`retend` 与账号 `retend666` 也只是形似 —— 归属只能由用户确认，不能推导。
...
> **⚠ 前置条件：只读路由必须先补上会话校验。**
>
> 中间件跑在 edge runtime **不能查数据库**，只能判断 `wallet_session` cookie
> 存不存在，判断不了有没有效。本文档「只读路由维持现状」那句在第一步成立，
> 因为共享口令还在前面挡着。
>
> 口令一拆，任何人手动设个 `wallet_session=随便什么` 就能过中间件，而
> `GET /api/tokens`、`/api/alerts` 这些只读路由不做二次校验，整个看板会被读出去。
> 第一步实施时用假 cookie 实测复现过。
>
> **这条不做，拆口令等于把看板公开。**
...
署名与账号是两个命名空间，无法从字符串推导对应关系。**以下映射全部由用户本人提供**，不是按相似度猜的（`retend` 与 `retend666` 形似，但采信的依据是用户确认，不是这个巧合）。

**代币**（2026-09-03 实测共 15 个，全部 `visibility='public'`）：

| 署名 | 归属账号 | 数量 | 依据 |
|---|---|---|---|
| `小牛` | `pananiu` | 4 | 用户告知 |
| `retend` | `retend666` | 2 | 用户告知 |
| NULL（无署名） | `pananiu` | 8 | 用户决定：都是账号系统上线前加的，归管理员 |
| `307大王小锐` | **保持 NULL** | 1 | 用户不确定是谁 |
...
|---|---|---|---|
| `小牛` | `pananiu` | 3 | 用户告知 |
| `赌命哥` | **保持 NULL** | 1 | 用户不确定是谁 |

一次性回填脚本 `scripts/backfill-owner.ts` 按上表写入，**不做任何字符串相似度匹配**。要点：

- **按署名值匹配，不按数量**。上表的数量是写文档时的实测值，不是断言；脚本跑的时候实际有几行就处理几行。看板一直在变（见下），把数量写死会让脚本在数据变动后行为错误
- 幂等：只写 `owner_id IS NULL` 的行，重复执行不覆盖后来的归属
- `307大王小锐` 与 `赌命哥` 那两条**不在映射表里**，因此不会被处理，保持无主 —— 只有管理员能删改
- 空字符串已核查：`created_by` 只有 NULL 和非空值两种，没有 `''` 或纯空白，所以 `IS NULL` 判定是完备的
...
- **`src/components/TokenActions.tsx`**：按权限渲染。非管理员且非 owner 时不显示「删除」「改备注」；非管理员不显示「冻结」「停用」。置顶按钮（`PinButton`）对所有登录用户保留。
  - **非 owner 看到的这一行会一个按钮都不剩，而且这是常态不是边缘情况**：回填后 15 个代币里 pananiu 占 12、retend666 占 2、无主 1，所以对 retend666 之外的任何普通用户，**15 个里有 15 个都没有可操作项**。这时不要渲染出一条空的操作区 —— 留个「只有添加者或管理员能改」的灰字说明，否则会被当成界面坏了。
