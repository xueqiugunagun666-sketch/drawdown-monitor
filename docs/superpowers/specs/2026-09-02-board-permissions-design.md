...
给共享看板加真实身份与删除权限：任何登录用户都能添加代币，但**只有管理员能删除任何一个**，普通用户只能删自己加的。停用与冻结这两个全局生效的操作收归管理员。日历（events）适用同一套规则。
...
全局报警档位（80/85/90/95%），改一次影响所有人的报警行为，比停用/冻结更全局。用户未就此表态。

**建议收归管理员**，但需确认后再实施。本文档不把它计入已确认范围。
...
5. 日历同构用例
6. 中间件：豁免路径可达；其他路径无 `wallet_session` 时被拦
...
| 只用共享口令、没注册账号的人被挡在外面 | **会**。这类人数量查不到（口令是无状态比对，服务端无记录）。这是本次改动的预期效果，他们需要注册 |
