...
暴涨引擎目标周期为 60 秒，每轮有 4,500 个唯一代币的有界预算。任务优先级如下：
...
各链并行取价和判定，单条慢链不会堵住其它链；元数据与历史回填进入独立的有界队列。
...
| 来源 | 用途 | 关键边界 |
|---|---|---|
| DexScreener | 实时价格、池子、流动性、成交量、项目链接 | 批量报价每请求最多 30 个地址；HTTP 200 空结果仍按失败处理 |
| XXYY | 批量候选价格与交叉验证 | 每批最多 500 个地址；缺文档保障，必须有健康监测和回退 |
...
10%，且该链本轮整体健康时，采用两者较低价。只有 XXYY 有价时不会单独继续判定；发生冲突时
当前版本回退 DexScreener，同时把“冲突应暂停”的候选结果写入 `quote_shadow` 做七天影子观察。
...
- DexScreener 的 30 地址批量上限仍是实时容量天花板之一；XXYY 不能替代流动性与成交量字段。
